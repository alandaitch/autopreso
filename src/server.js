...
export async function runWhiteboardAgent({ transcript, state, wss, options, generateTextFn = generateText, streamTextFn = streamText }) {
  ...
  // Model has returned, with or without a draw. Lock the turn in so the
  // queue's cancel window closes — a follow-up chunk shouldn't kill a
  // turn that already produced its (possibly empty) answer.
